 ...
 * 스프링은 직접 클래스를 생성하는 것을 지양하고 인터페이스르 통해 접근하는 것을 권장하는 프레임워크이다.
 */
@Service
public class BookServiceImpl implements BookService {
	 ...
	 * 클래스의 이름이 Impl로 끝나는 것은 implements의 약자로 관습에 따른다(개발자들 사이에서)
	...
	public String insertBook(Map<String, Object> map) {
		...
		// insert 구문은 입력이 성고하면 1, 실패하면 0을 리턴한다.
		int affectRowCount = bookDao.insert(map);
		// book_id = 34
		if(affectRowCount == 1) {
			
			// 결과가 성공일 시, map 인스터스에 book 테이블의 pk인 book_id가 담겨있다.
	...
	public boolean deleteBook(Map<String, Object> map) {
		// 삭제의 경우 수정과 동일하게 한 개의 행이 제대로 영향 받았는지만 검사하면 된다.
